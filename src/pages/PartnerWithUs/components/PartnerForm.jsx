...
function PartnerForm() {
  ...
  return (
    <div className="relative bg-white p-5 pb-10 lg:p-10 w-full md:w-[clamp(30rem,99%,50rem)] rounded-2xl shadow-lg lg:shadow-xl shadow-brown-gray/50 flex flex-col justify-between h-fit">
      ...
      <form className="z-[2] py-4" onSubmit={handleSubmit(onSubmit, onError)}>
        <div className="flex flex-col xl:flex-row">
          <label htmlFor="name" className="flex flex-col p-2 mb-5 justify-start w-full">
            Name:
            <input {...register("name")} type="text" id="name" className={`bg-white rounded-md p-2 w-full`} />
          </label>
          <label htmlFor="email" className="flex flex-col p-2 mb-5 justify-start w-full">
            Email:
            <input {...register("email")} type="email" id="email" className={`bg-white rounded-md p-2 w-full`} />
          </label>
        </div>
        <label htmlFor="occupation" className="flex flex-col p-2 mb-5 justify-start relative">
          <span>Tell us what you do:</span><div className="bg-[url('assets/chevron-bottom.svg')] bg-no-repeat bg-center bg-contain w-4 h-4 right-[1.2rem] bottom-[1.125rem] absolute z-[2] pointer-events-none" role="img" aria-roledescription="icon" />
          <select {...register("occupation")} name="occupation" id="occupation" className={`bg-white
                rounded-md
                p-2
